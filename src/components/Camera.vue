<template>
  <transition name="slide"
    ...
    <div class="camera"
      ...
      <canvas ref="canvas"
              v-show="isPreviewing"
              :style="currentFilter"></canvas>
      <div class="camera_mask"></div>
      <div class="camera_filter">
        <ul :style="{ width: `${filters.length * 100}%` }">
          <li v-for="(filter, index) in filters"
              :key="index"
              :class="{ camera_filter_index: index === currentIndex }"
              :style="{ transform: `translateX(-${currentIndex * 100}%)` }">
            {{ filter.name }}
          </li>
        </ul>
      </div>
      <button class="camera_btn-cancel"
              v-show="isPreviewing"
              @click="resetCamera">
        <i class="fa fa-chevron-left"></i>
      </button>
      <button class="camera_btn-save"
              v-show="isPreviewing"
              @click="savePicture">
        <i class="fa fa-check"></i>
      </button>
      <button class="camera_btn-close"
              v-show="!isPreviewing"
              @click="toggleCamera">
        <i class="fa fa-close"></i>
      </button>
      <button class="camera_btn-take"
              v-show="!isPreviewing"
              @click="takePicture">
      </button>
    </div>
